{{-- Asset Purchase Order Information Injection --}}
{{-- This script injects PO information into asset detail pages --}}
...
function injectPurchaseOrderInfo(po) {
    ...
    const poInfoHtml = `
        <div class="row" id="purchase-order-info">
            <div class="col-md-12">
    ...
    // Find a good place to inject the PO info (after the asset info box)
    const targetElement = $('.box').first();
    if (targetElement.length) {
        targetElement.after(poInfoHtml);
    } else {
        // Fallback: add to main content area
        $('.content-wrapper .content').prepend(poInfoHtml);
    }
}
</script>
