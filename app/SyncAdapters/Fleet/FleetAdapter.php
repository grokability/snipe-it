...
class FleetAdapter extends ConfigurableAdapter
{
    ...
    /**
     * Push Snipe-IT-authoritative fields to Fleet as manual labels.
     * Fleet has no native asset_tag / notes columns, so each push-
     * directed field becomes a per-host manual label named
     * `snipeit_{field}_{value}`. Membership is set via label-spec so
     * one API call creates the label (if new) and sets its member
     * host in a single round trip.
     *
     * Uses the asset's serial number to identify the host to Fleet
     * because serial is more stable than hostname across renames
     * and re-enrollments. Silently no-ops when the asset has no
     * serial (nothing to identify the Fleet host with).
     *
    ...
    public function push(Asset $asset, array $changedFields = []): void
    {
        ...
        foreach ($pushFields as $field) {
            ...
            $labelName = self::labelNameFor($field, $value);
            $client->upsertManualLabelForHost($labelName, $serial);

            Log::channel('sync-adapters')->info(sprintf(
                '%s push: set label "%s" on Fleet host serial=%s',
                $this->name(),
                $labelName,
                $serial,
            ));
    ...
    /**
     * Build the Fleet label name for a Snipe-IT source-field / value
     * pair. `snipeit_` prefix so admins can filter Fleet's label list
     * down to the sync-managed ones. Underscore separator between
     * field name and value because Fleet's label names allow letters,
     * digits, underscores, and hyphens but not colons or slashes.
     * Value is sanitized to the same charset.
     */
    private static function labelNameFor(string $field, string $value): string
    {
        return 'snipeit_'.$field.'_'.preg_replace('/[^A-Za-z0-9_\\-]/', '_', $value);
    }

    /**
     * Resolve a Snipe-IT source of truth for a given source-field
     * name. Currently only asset_tag is push-supported for Fleet
     * (the vendor field most commonly requested); additional fields
     * add cases here as we wire them.
     */
    private function assetValueForSourceField(Asset $asset, string $field): ?string
    {
        return match ($field) {
